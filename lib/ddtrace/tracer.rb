...
module Datadog
  ...
  class Tracer
    ...
    def build_span(name, options = {})
      ...
      span = Datadog::SpanOperation.build(self, context, name, options)
      ...
      span.set_tags(options[:tags]) if options.key?(:tags)
  ...
  class SpanOperation
    ...
    def self.build(tracer, context, span_name, span_options = {})
      parent = span_options[:child_of]
      span_options[:service] ||= parent.service unless parent.nil?
      ...
      # Wrap it in an operation
      op = new(tracer, context, span)
      op.context.add_span(op) if op.context
      ...
      op.parent = parent unless parent.nil?
      op
    end

    def initialize(tracer, context, span)
      @tracer = tracer
      @context = context
      @span = span
    end
